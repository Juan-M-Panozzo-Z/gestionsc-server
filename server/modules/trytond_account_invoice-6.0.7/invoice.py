...
class Invoice(Workflow, ModelSQL, ModelView, TaxableMixin):
    ...
    tax_identifier = fields.Many2One(
        "party.identifier", "Tax Identifier", states=_states, depends=_depends
    )
    ...
    reference = fields.Char("Reference", size=None, states=_states, depends=_depends)
    description = fields.Char(
        "Description", size=None, states=_states, depends=_depends
    )
    ...
    accounting_date = fields.Date("Accounting Date", states=_states, depends=_depends)
    ...
    party_lang = fields.Function(
        fields.Char("Party Language"), "on_change_with_party_lang"
    )
    ...
    currency = fields.Many2One(
        ...
        required=True,
        states={
            "readonly": (_states["readonly"] | (Eval("lines", [0]) & Eval("currency"))),
        },
        depends=_depends,
    )
    ...
    currency_date = fields.Function(
        fields.Date("Currency Date"), "on_change_with_currency_date"
    )
    ...
    account = fields.Many2One(
        ...
        required=True,
        states=_states,
        depends=_depends + ["type", "company", "accounting_date", "invoice_date"],
        domain=[
            ("closed", "!=", True),
            ("company", "=", Eval("company", -1)),
            If(
        ...
        context={
            "date": If(
                Eval("accounting_date"), Eval("accounting_date"), Eval("invoice_date")
            ),
        },
    )
    payment_term = fields.Many2One(
        "account.invoice.payment_term", "Payment Term", states=_states, depends=_depends
    )
    lines = fields.One2Many(
        ...
        states={
            "readonly": (
                (Eval("state") != "draft") | ~Eval("company") | ~Eval("currency")
            ),
        },
        depends=["state", "company", "currency", "type", "party"],
    )
    taxes = fields.One2Many(
        "account.invoice.tax", "invoice", "Tax Lines", states=_states, depends=_depends
    )
    ...
    tax_amount = fields.Function(
        fields.Numeric(
            "Tax", digits=(16, Eval("currency_digits", 2)), depends=["currency_digits"]
        ),
        "get_amount",
    ...
    lines_to_pay = fields.Function(
        fields.Many2Many("account.move.line", None, None, "Lines to Pay"),
        "get_lines_to_pay",
    )
    payment_lines = fields.Many2Many(
        ...
        domain=[
            ...
            If(
                Eval("type") == "out",
                If(Eval("total_amount", 0) >= 0, ("debit", "=", 0), ("credit", "=", 0)),
                If(Eval("total_amount", 0) >= 0, ("credit", "=", 0), ("debit", "=", 0)),
            ),
        ],
        states={
    ...
    amount_to_pay_today = fields.Function(
        fields.Numeric(
            "Amount to Pay Today",
            digits=(16, Eval("currency_digits", 2)),
            depends=["currency_digits"],
    ...
    invoice_report_cache = fields.Binary(
        "Invoice Report", readonly=True, file_id=file_id, store_prefix=store_prefix
    )
    ...
    # invoice_selector = character varying
    ...
    def __setup__(cls):
        ...
        cls._buttons.update(
            {
                ...
                "draft": {
                    "invisible": (
                        ...
                        | ((Eval("state") == "cancelled") & Eval("cancel_move", -1))
                    ),
                    "icon": If(
                        Eval("state") == "cancelled", "tryton-undo", "tryton-back"
                    ),
                    "depends": ["state"],
                },
                "validate_invoice": {
        ...
        cls.__rpc__.update(
            {
                "post": RPC(readonly=False, instantiate=0, fresh_session=True),
            }
        )
    ...
    def __register__(cls, module_name):
        ...
        cursor.execute(
            *sql_table.update(
                [sql_table.state], ["cancelled"], where=sql_table.state == "cancel"
            )
        )
    ...
    def get_type_name(cls, invoices, name):
        type_names = {}
        type2name = {}
        for type, name in cls.fields_get(fields_names=["type"])["type"]["selection"]:
            type2name[type] = name
    ...
    def _on_change_lines_taxes(self):
        ...
        for tax in self.taxes or []:
            ...
            if (key not in computed_taxes) or (key in tax_keys):
                ...
                continue
            tax_keys.append(key)
            if not is_zero(computed_taxes[key]["base"] - (tax.base or Decimal("0.0"))):
    ...
    def get_amount(cls, invoices, names):
        ...
        if to_round:
            for invoice in invoices:
                tax_amount[invoice.id] = invoice.currency.round(tax_amount[invoice.id])
        ...
        for sub_ids in grouped_slice(invoices_move):
            ...
            cursor.execute(
                *invoice.join(move, condition=invoice.move == move.id)
        ...
        for invoice in invoices_no_move:
            untaxed_amount[invoice.id] = sum(
                (line.amount for line in invoice.lines if line.type == "line"),
                Decimal(0),
            )
            total_amount[invoice.id] = (
        ...
        for sub_ids in grouped_slice(invoices):
            ...
            cursor.execute(
                *invoice.join(
                    line,
                    condition=(
                        (invoice.move == line.move) & (invoice.account == line.account)
                    ),
                ).select(
                    invoice.id,
                    line.id,
        ...
        for invoice in invoices:
            ...
            for line in invoice.lines_to_pay:
                if line.reconciliation:
                    continue
                if name == "amount_to_pay_today" and (
                    not line.maturity_date or line.maturity_date > today
                ):
                    continue
                if line.second_currency and line.second_currency == invoice.currency:
    ...
    def search_total_amount(cls, name, clause):
        ...
        union = line.join(invoice, condition=(invoice.id == line.invoice)).join(
            currency, condition=(currency.id == invoice.currency)
        ).select(
        ...
        query = union.select(
            ...
            having=Operator(Sum(union.total_amount).cast(type_name), value),
        )
        return [("id", "in", query)]
    ...
    def search_untaxed_amount(cls, name, clause):
        ...
        query = (
            line.join(invoice, condition=(invoice.id == line.invoice))
    ...
    def search_tax_amount(cls, name, clause):
        ...
        query = tax.select(
            ...
            where=tax.invoice.in_(invoice_query),
            group_by=tax.invoice,
            having=Operator(Coalesce(Sum(tax.amount), 0).cast(type_name), value),
        )
        return [("id", "in", query)]
        ...
        for invoice in invoices:
            ...
            if exception and (to_create or to_delete or to_write):
                raise InvoiceTaxValidationError(
                    gettext(
                        "account_invoice.msg_invoice_tax_invalid",
    ...
    def _get_move_line(self, date, amount):
        ...
        if line.amount_second_currency:
            line.amount_second_currency = line.amount_second_currency.copy_sign(
    ...
    def get_move(self):
        ...
        if any(past_payment_term_dates):
            ...
            if Warning.check(warning_key):
                raise InvoicePaymentTermDateWarning(
                    warning_key,
                    gettext(
                        "account_invoice" ".msg_invoice_payment_term_date_past",
        ...
        for invoice in invoices:
            ...
            if invoice.type == "out" and invoice.sequence not in sequences:
                ...
                after_invoices = cls.search(
                    ...
                    limit=1,
                    order=[("accounting_date", "DESC")],
                )
                if after_invoices:
                    (after_invoice,) = after_invoices
                    raise InvoiceNumberError(
                        gettext(
                            "account_invoice.msg_invoice_number_after",
                            invoice=invoice.rec_name,
    ...
    def get_next_number(self, pattern=None):
        ...
        period_id = Period.find(
            self.company.id, date=accounting_date, test_state=self.type != "in"
        )
        ...
        for invoice_sequence in fiscalyear.invoice_sequences:
            if invoice_sequence.match(pattern):
                sequence = getattr(invoice_sequence, self._sequence_field)
                break
        else:
            raise InvoiceNumberError(
                gettext(
                    "account_invoice.msg_invoice_no_sequence",
                    invoice=self.rec_name,
    ...
    def _sequence_field(self):
        ...
        field = self.type
        if (
            all(l.amount <= 0 for l in self.lines if l.product)
    ...
    def check_modify(cls, invoices):
        ...
        for invoice in invoices:
            if not invoice.is_modifiable:
                raise AccessError(
                    gettext(
                        "account_invoice.msg_invoice_modify", invoice=invoice.rec_name
                    )
                )
    ...
    def view_attributes(cls):
        return super().view_attributes() + [
            ...
            (
                ...
                "visual",
                If(
                    ((Eval("type") == "out") & (Eval("amount_to_pay_today", 0) > 0))
                    | ((Eval("type") == "in") & (Eval("amount_to_pay_today", 0) < 0)),
                    "danger",
    ...
    def check_same_account(self):
        for line in self.lines:
            if line.type == "line" and line.account == self.account:
    ...
    def check_payment_lines(self):
        ...
        if abs(amount) < abs(payment_amount):
            raise InvoiceValidationError(
                gettext(
                    "account_invoice" ".msg_invoice_payment_lines_greater_amount",
                    invoice=self.rec_name,
                )
            )
    ...
    def get_reconcile_lines_for_amount(self, amount):
        ...
        lines = [
            l for l in self.payment_lines + self.lines_to_pay if not l.reconciliation
        ]
    ...
    def pay_invoice(
        ...
        if overpayment:
            ...
            overpayment_line.debit = abs(overpayment) if pay_line.debit else 0
            overpayment_line.credit = abs(overpayment) if pay_line.credit else 0
        ...
        counterpart_line.account = getattr(payment_method, payment_acccount).current(
            date=date
        )
        ...
        payment_line = [
            ...
            if (l.debit, l.credit) == (pay_line.debit, pay_line.credit)
        ][0]
    ...
    def add_payment_lines(cls, payments):
        ...
        for invoice, lines in payments.items():
            if invoice.state == "paid":
                raise AccessError(
                    gettext(
                        "account_invoice" ".msg_invoice_payment_lines_add_remove_paid",
                        invoice=invoice.rec_name,
                    )
                )
        ...
        if refund:
            ...
            for invoice, new_invoice in zip(invoices, new_invoices):
                if invoice.state != "posted":
                    raise AccessError(
                        gettext(
                            "account_invoice" ".msg_invoice_credit_refund_not_posted",
                            invoice=invoice.rec_name,
                        )
                    )
        ...
        for invoice in invoices:
            if invoice.move or invoice.number:
                if invoice.move and invoice.move.state == "draft":
                    ...
                    if invoice.type == "out" and not invoice.company.cancel_invoice_out:
                        raise AccessError(
                            gettext(
                                "account_invoice" ".msg_invoice_customer_cancel_move",
                                invoice=invoice.rec_name,
                            )
                        )
                    if invoice.move:
...
class InvoicePaymentLine(ModelSQL):
    ...
    invoice = fields.Many2One(
        "account.invoice", "Invoice", ondelete="CASCADE", select=True, required=True
    )
    invoice_account = fields.Function(
        fields.Many2One("account.account", "Invoice Account"), "get_invoice"
    )
    ...
    def get_invoice(cls, records, names):
        ...
        for record in records:
            if invoice_account:
                result["invoice_account"][record.id] = record.invoice.account.id
            if invoice_party:
...
class InvoiceLine(sequence_ordered(), ModelSQL, ModelView, TaxableMixin):
    ...
    currency = fields.Many2One(
        "currency.currency", "Currency", required=True, states=_states, depends=_depends
    )
    currency_digits = fields.Function(
        fields.Integer("Currency Digits"), "on_change_with_currency_digits"
    )
    ...
    unit_price = fields.Function(
        fields.Numeric("Precio unitario"),
    ...
    amount = fields.Function(
        fields.Numeric(
            ...
            digits=(
                16,
                Eval("_parent_invoice", {}).get(
                    "currency_digits", Eval("currency_digits", 2)
                ),
            ),
            states={
    ...
    invoice_taxes = fields.Function(
        fields.Many2Many("account.invoice.tax", None, None, "Invoice Taxes"),
        "get_invoice_taxes",
    )
    origin = fields.Reference(
        "Origin", selection="get_origin", select=True, states=_states, depends=_depends
    )
    ...
    especialista_unit = fields.Function(
        fields.Integer(
            "especialista",
        ),
    ...
    invoice_selector = fields.Function(
    ...
    type_unit_price = fields.Boolean("Precio unitario manual")
    # fin personalizados
            ...
            If(
                ...
                If(
                    ...
                    If(
                        ...
                        ["OR", cls._account_domain("out"), cls._account_domain("in")],
                    ),
                ),
                If(
                    Eval("invoice_type") == "out",
    ...
    def __register__(cls, module_name):
        ...
        cursor.execute(
            *sql_table.join(invoice, condition=sql_table.invoice == invoice.id).select(
                sql_table.id, invoice.company, where=sql_table.company == Null
            )
        )
        for line_id, company_id in cursor:
            update.execute(
                *sql_table.update(
                    [sql_table.company], [company_id], where=sql_table.id == line_id
                )
            )
    ...
    @classmethod
    def default_unit_price_manual(cls):
        return 0
    ...
    def on_change_with_amount(self):
        if self.type == "line":
            currency = self.invoice.currency if self.invoice else self.currency
            ...
            if (
                ...
                with Transaction().set_context(_deductible_rate=1):
                    tax_amount = sum(t["amount"] for t in self._get_taxes().values())
                non_deductible_amount = tax_amount * (1 - self.taxes_deductible_rate)
    ...
    def taxable_lines(self):
        ...
        if getattr(self, "invoice", None) and getattr(self.invoice, "type", None):
        ...
        return [
            (
                ...
                ((getattr(self, "unit_price", None) or Decimal(0)) * deductible_rate),
    ...
    @fields.depends(
        "invoice", "_parent_invoice.accounting_date", "_parent_invoice.invoice_date"
    )
    ...
    def create(cls, vlist):
        ...
        for invoice in Invoice.browse(invoice_ids):
            if invoice.state in ("posted", "paid", "cancelled"):
                raise AccessError(
                    gettext(
                        "account_invoice.msg_invoice_line_create",
                        invoice=invoice.rec_name,
                    )
                )
    ...
    def check_same_account(self):
        if self.type == "line":
            if self.invoice and self.account == self.invoice.account:
        ...
        for tax in taxes:
            ...
            with Transaction().set_context(date=self.invoice.currency_date):
                amount = Currency.compute(
                    self.invoice.currency, amount, self.invoice.company.currency
                )
            tax_line = TaxLine()
    ...
    def get_move_lines(self):
        ...
        if self.invoice.currency != self.invoice.company.currency:
            with Transaction().set_context(date=self.invoice.currency_date):
                amount = Currency.compute(
                    self.invoice.currency, self.amount, self.invoice.company.currency
                )
...
class InvoiceLineTax(ModelSQL):
    ...
    tax = fields.Many2One("account.tax", "Tax", ondelete="RESTRICT", required=True)
...
class InvoiceTax(sequence_ordered(), ModelSQL, ModelView):
    ...
    description = fields.Char(
        "Description", size=None, required=True, states=_states, depends=_depends
    )
    sequence_number = fields.Function(
        fields.Integer("Sequence Number"), "get_sequence_number"
    )
    account = fields.Many2One(
        "account.account",
        "Account",
    ...
    currency = fields.Function(
        fields.Many2One("currency.currency", "Currency"), "on_change_with_currency"
    )
    ...
    legal_notice = fields.Text("Legal Notice", states=_states, depends=_depends)
    ...
    @fields.depends(
        "tax", "base", "amount", "manual", "invoice", "_parent_invoice.currency"
    )
    def on_change_with_amount(self):
        Tax = Pool().get("account.tax")
    ...
    def create(cls, vlist):
        ...
        for invoice in Invoice.browse(invoice_ids):
            if invoice.state in ("posted", "paid", "cancelled"):
                raise AccessError(
                    gettext(
                        "account_invoice.msg_invoice_tax_create",
                        invoice=invoice.rec_name,
                    )
                )
...
class InvoiceReport(Report):
    ...
    def _execute(cls, records, header, data, action):
        ...
        if invoice.invoice_report_cache:
            return (invoice.invoice_report_format, invoice.invoice_report_cache)
        else:
            ...
            if invoice.state in {"posted", "paid"} and invoice.type == "out":
                ...
                invoice.invoice_report_cache = Invoice.invoice_report_cache.cast(data)
                invoice.save()
            return result
...
class PayInvoiceAsk(ModelView):
    ...
    currency = fields.Many2One("currency.currency", "Payment Currency", readonly=True)
    currency_digits = fields.Integer("Payment Currency Digits", readonly=True)
    amount_writeoff = fields.Numeric(
        "Write-Off Amount",
        digits=(16, Eval("currency_digits_writeoff", 2)),
        readonly=True,
        depends=["currency_digits_writeoff", "type"],
        states={
            "invisible": Eval("type") != "writeoff",
        },
    )
    currency_writeoff = fields.Many2One(
        "currency.currency",
        "Write-Off Currency",
        readonly=True,
        states={
    ...
    currency_digits_writeoff = fields.Integer(
        "Write-Off Currency Digits", required=True, readonly=True
    )
    lines_to_pay = fields.Many2Many(
        "account.move.line", None, None, "Lines to Pay", readonly=True
    )
    ...
    def on_change_lines(self):
        ...
        if self.currency and self.currency_writeoff:
            with Transaction().set_context(date=self.date):
                amount = Currency.compute(
                    self.currency, self.amount, self.currency_writeoff
                )
        else:
            amount = self.amount
...
class PayInvoice(Wizard):
    ...
    def transition_choice(self):
        ...
        with Transaction().set_context(date=self.start.date):
            amount = Currency.compute(
                self.start.currency, self.start.amount, invoice.company.currency
            )
        ...
        if remainder == Decimal("0.0") and amount_invoice <= invoice.amount_to_pay:
            return "pay"
        return "ask"
    ...
    def default_ask(self, fields):
        ...
        if amount_invoice > invoice.amount_to_pay or invoice.company.currency.is_zero(
            amount
        ):
    ...
    def transition_pay(self):
        ...
        overpayment = 0
        if (
            0 <= invoice.amount_to_pay < amount_invoice
        ...
        if remainder:
            if self.ask.type != "partial":
                ...
                if to_reconcile:
                    MoveLine.reconcile(
                        to_reconcile, writeoff=self.ask.writeoff, date=self.start.date
                    )
        else:
...
class CreditInvoice(Wizard):
    ...
    def do_credit(self, action):
        credit_invoices = self.model.credit(self.records, **self._credit_options)
