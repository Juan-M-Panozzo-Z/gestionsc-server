...
class Configuration(metaclass=PoolMeta):
    ...
    default_customer_payment_term = fields.MultiValue(
        fields.Many2One("account.invoice.payment_term", "Default Customer Payment Term")
    )
...
class FiscalYear(metaclass=PoolMeta):
    ...
    def __register__(cls, module_name):
        ...
        if (
            table.column_exist("in_invoice_sequence")
            ...
            cursor.execute(
                *sequence.insert(
                    ...
                    values=sql_table.join(
                        fiscalyear, condition=(fiscalyear.id == sql_table.fiscalyear)
                    ).select(
                        Literal(10),
                        sql_table.fiscalyear,
                        fiscalyear.company,
                        sql_table.id,
                        Coalesce(
                            sql_table.out_invoice_sequence,
    ...
    def close(cls, periods):
        ...
        if invoices:
            ...
            raise ClosePeriodError(
                gettext(
                    "account_invoice.msg_close_period_non_posted_invoices",
                    invoices=names,
                )
            )
        super().close(periods)
...
class InvoiceSequence(sequence_ordered(), ModelSQL, ModelView, MatchMixin):
    ...
    in_invoice_sequence = fields.Many2One(
        "ir.sequence.strict",
    ...
    in_credit_note_sequence = fields.Many2One(
        "ir.sequence.strict",
    ...
    out_invoice_sequence = fields.Many2One(
        "ir.sequence.strict",
    ...
    out_credit_note_sequence = fields.Many2One(
        "ir.sequence.strict",
...
class MoveLine(metaclass=PoolMeta):
    ...
    invoice_payment = fields.Function(
        fields.Many2One(
            "account.invoice",
            "Invoice Payment",
            domain=[
    ...
    @classmethod
    def _get_origin(cls):
        return super()._get_origin() + ["account.invoice.line", "account.invoice.tax"]
    ...
    def product(self):
        ...
        if isinstance(self.origin, InvoiceLine) and self.origin.product:
...
class RenewFiscalYear(metaclass=PoolMeta):
    ...
    def create_fiscalyear(self):
        ...
        for previous, new in zip(
            filter(standard_period, self.start.previous_fiscalyear.periods),
        ...
        copies = Sequence.copy(
            ...
            default={
                ...
                "name": lambda data: data["name"].replace(
                    self.start.previous_fiscalyear.name, self.start.name
                ),
            },
        )
